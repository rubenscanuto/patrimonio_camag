                                ...
                                <div className="flex-1 overflow-y-auto p-6 bg-slate-50">
                                    {ownerModalTab === 'Data' ? (
                                        ...
                                        <div className="space-y-6">
                                            {/* Documents Tab Content (existing) */}
                                            <div 
                                                className="border-2 border-dashed border-slate-300 rounded-xl p-8 flex flex-col items-center justify-center text-slate-500 hover:bg-slate-100 hover:border-indigo-400 transition-all cursor-pointer bg-white"
                                            ...
                                            {pendingFiles.length > 0 && (
                                                <div className="bg-white rounded-xl border border-slate-200 p-4">
                                                    <h4 className="font-bold text-sm text-slate-700 mb-3">Arquivos para Envio</h4>
                                                    <ul className="space-y-2">
                                                        {pendingFiles.map((file, idx) => (
                                                            <li key={idx} className="flex justify-between items-center text-sm bg-slate-50 p-2 rounded">
                                                                <span className="truncate flex-1">{file.name}</span>
                                                                <button onClick={() => setPendingFiles(prev => prev.filter((_, i) => i !== idx))} className="text-red-500 hover:bg-red-50 p-1 rounded"><X size={14}/></button>
                                                            </li>
                                                        ))}
                                                    </ul>
                                                    <div className="flex gap-2 mt-4 justify-end">
                                                        <button type="button" onClick={() => handleUploadAction(false)} disabled={isAnalyzingDoc} className="text-xs bg-white border border-slate-300 px-3 py-2 rounded hover:bg-slate-50 text-slate-700">Apenas Anexar</button>
                                                        <button type="button" onClick={() => handleUploadAction(true)} disabled={isAnalyzingDoc} className="text-xs bg-indigo-600 text-white px-3 py-2 rounded hover:bg-indigo-700 flex items-center gap-2">
                                                            {isAnalyzingDoc ? <Loader2 className="animate-spin" size={14}/> : <Sparkles size={14}/>} Processar com IA
                                                        </button>
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            <div className="space-y-3">
                                                <h4 className="font-bold text-sm text-slate-700">Documentos Cadastrados</h4>
                                                {getOwnerDocuments().length === 0 ? (
                                                    ...
                                                    getOwnerDocuments().map(doc => (
                                                        <div key={doc.id} className="flex items-center justify-between bg-white p-3 rounded border border-slate-200 shadow-sm">
